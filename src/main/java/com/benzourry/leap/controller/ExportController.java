...
//        String filename = dataset.getTitle().replace(" ", "-").toLowerCase();
    ...
    @RequestMapping(value = "/report/export-data/{id}/json", method = RequestMethod.GET)
    @ResponseBody
    public List<EntryDto> getMyData(HttpServletRequest request,
                                  HttpServletResponse response,
                                  @PathVariable("id") Long id,
                                  @RequestParam(value = "email", required = false) String email,
                                  @RequestParam(value = "searchText", required = false) String searchText,
...
//    }
    ...
    @PostMapping("/api/import/entry/{formId}/{format}")
    public @ResponseBody Map<String, Object> importDataToEntry(
            ...
            @RequestParam(value = "import-live", defaultValue = "false") boolean importToLive) throws IOException {
        if ("xlsx".equals(format)) {
            return mapExcelDataToEntry(formId, dataFile, email, create, createDataset, createDashboard, importToLive);
        } else if ("json".equals(format)) {
            return mapJsonDataToEntry(formId, dataFile, importToLive);
        } else {
            Map<String, Object> result = new HashMap<>();
            result.put("success", false);
    ...
    public Map<String, Object> mapExcelDataToEntry(
            Long formId,MultipartFile reapExcelDataFile,String email,
            boolean create,boolean createDataset,boolean createDashboard,boolean importToLive) throws IOException {
    ...
    public Map<String, Object> mapJsonDataToEntry(
            Long formId, MultipartFile jsonFile, boolean importToLive) {
        ...
        try (InputStream inputStream = jsonFile.getInputStream()) {
            ...
            // 2. Read the JSON file into a JsonNode using ObjectMapper
            ...
            // 3. Iterate through JSON objects and construct Entry entities
            for (JsonNode node : rootNode) {
                Entry entry = new Entry();
                entry.setForm(form);
                entry.setLive(importToLive);

                // Extract standard fields from the JSON object
                entry.setEmail(node.hasNonNull("email") ? node.get("email").asText() : null);
                entry.setCurrentStatus(node.hasNonNull("currentStatus") ? node.get("currentStatus").asText() : Entry.STATUS_DRAFTED);

                if (node.hasNonNull("submissionDate")) {
                    entry.setSubmissionDate(new Date(node.get("submissionDate").asLong()));
                } else {
                    entry.setSubmissionDate(new Date());
                }

                // Directly assign the entire "data" JSON node to the entry
                if (node.hasNonNull("data")) {
                    entry.setData(node.get("data"));
                } else {
                    entry.setData(MAPPER.createObjectNode());
                }
            ...
            // 4. Update Form counter and Save
            form.setCounter(counter);
            formRepository.save(form);
            entryRepository.saveAll(tempEntryList);
            ...
            logs.add(new String[]{e.getMessage(), "danger", "Failed"});
            TenantLogger.error(form.getAppId(), "form", formId, "Error importing JSON data: " + e.getMessage());
            result.put("success", false);
            result.put("message", "Failed to parse or save JSON data: " + e.getMessage());
        }
