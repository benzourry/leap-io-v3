...
//            "where f.id = :formId and s.type in :sectionType and i.datasource is not null", nativeQuery = true)
    ...
    List<Map> findIdByFormIdAndSectionType(@Param("formId") Long formId,@Param("sectionType") List<String> sectionType);
    ...
    @Query("select l from Lookup l where l.app.id = :appId" +
            " AND (upper(l.name) like :searchText or upper(l.description) like :searchText)")
    Page<Lookup> findByAppId(@Param("searchText") String searchText,@Param("appId") long appId, Pageable pageable);
...
//            " left join user_group access on l.access = access.id " +
            ...
            " AND (upper(l.name) like :searchText or upper(l.description) like :searchText)")
    Page<Lookup> findByIdsAndEmail(@Param("searchText") String searchText,@Param("ids") List<Long> ids,@Param("email") String email, Pageable pageable);
