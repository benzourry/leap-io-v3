...
//    }
    ...
    public Page<EntryDto> findPagedOld(Specification<Entry> spec, Map<String, Set<String>> fields, Pageable pageable){
        CriteriaBuilder cb = em.getCriteriaBuilder();
...
// Build JSON field selections
        ...
        fields.get("$").forEach(vkeys -> {
            dataJsonArgs.add(cb.literal(vkeys));
            dataJsonArgs.add(cb.function("JSON_VALUE", String.class, root.get("data"), cb.literal("$." + vkeys)));
        });


        // Get the LEFT JOIN from Specification
        Join<Entry, Entry> prevEntryJoin = root.getJoins()
                .stream()
                .filter(j -> j.getAttribute().getName().equals("prevEntry"))
                .map(j -> (Join<Entry, Entry>) j)
                .findFirst()
                .orElse(root.join("prevEntry", JoinType.LEFT)); // fallback

        List<Expression<?>> prevJsonArgs = new ArrayList<>();
        fields.get("$prev$").forEach(vkeys -> {
            prevJsonArgs.add(cb.literal(vkeys));
            prevJsonArgs.add(cb.function("JSON_VALUE", String.class, prevEntryJoin.get("data"), cb.literal("$." + vkeys)));
        });

        Expression<Object> prevJson = cb.selectCase()
                        .when(cb.isNotNull(prevEntryJoin.get("id")),
                                cb.function(
                                        "JSON_OBJECT",
                                        JsonNode.class,
                                        prevJsonArgs.toArray(new Expression[0])
                                )
                        )
                        .otherwise(cb.nullLiteral(JsonNode.class));
        ...
        cq.select(cb.construct(
                ...
                cb.function(
                        "JSON_OBJECT",
                        JsonNode.class,
                        dataJsonArgs.toArray(new Expression[0])
                ),
                prevJson,
                root.get("formId"),
        ...
        List<Object[]> approvalRows = em.createQuery("""
                    SELECT a.entry.id, a.tierId, a
                    FROM EntryApproval a
                    WHERE a.entry.id IN :ids
                """)
                .setParameter("ids", ids)
                .getResultList();

        List<Object[]> approverRows = em.createQuery("""
                    SELECT e.id, KEY(ap), ap
                    FROM Entry e
                    JOIN e.approver ap
                    WHERE e.id IN :ids
                """)
        ...
        Map<Long, Map<Long, EntryApproval>> approvals = new HashMap<>();
        Map<Long, Map<Long, String>> approvers = new HashMap<>();
        ...
        for (EntryDto dto : resultList) {
            dto.setApproval(
                    approvals.getOrDefault(dto.getId(), Map.of())
            );
            dto.setApprover(
                    approvers.getOrDefault(dto.getId(), Map.of())
            );
        }
    ...
    public Page<EntryDto> findPaged(Specification<Entry> spec, Map<String, Set<String>> fields, Pageable pageable) {
        CriteriaBuilder cb = em.getCriteriaBuilder();
        CriteriaQuery<EntryDto> cq = cb.createQuery(EntryDto.class);
        Root<Entry> root = cq.from(Entry.class);
